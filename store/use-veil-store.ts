...
type VeilState = {
  ...
  audioLevel: number;
  /** Music / Deezer / YouTube wall visuals — persists while relay inbox is open on top. */
  ambientPreview: MediaPreview | null;
  /** Messaging MCP relay dock — renders above ambient preview. */
  ...
  /** @deprecated Prefer openAmbientPreview / openRelayPreview. */
  openPreview: (preview: MediaPreview) => void;
  closeAmbientPreview: () => void;
  closeRelayPreview: () => void;
  /** Closes whichever preview surface is closest (relay first via modal wiring). */
  closePreview: () => void;
  setActiveRoom: (room: string | null) => void;
  setColonyCrowdMode: (mode: VeilState["colonyCrowdMode"]) => void;
  openCrewDeck: () => void;
  closeCrewDeck: () => void;
  toggleCrewDeck: () => void;
...
export const useVeilStore = create<VeilState>()(
  ...
  nodes: [
    ...
    { id: "veil", label: "VEIL", status: "idle", x: 50, y: 42 },
    { id: "software", label: "Software layer", status: "idle", x: 80, y: 58 }
  ],
  edges: [
    { id: "human-veil", from: "human", to: "veil", status: "queued" },
    { id: "veil-software", from: "veil", to: "software", status: "queued" }
  ],
  timeline: [],
  memoryCards: [
    {
  ...
  insights: [
    ...
    },
    {
      ...
      detail: "VEIL can prepare a brief from calendar, notes, and message context.",
      mood: "focus"
    }
  ],
  audioLevel: 0,
  ...
  interrupt: () =>
...
}),
    {
      ...
      merge: (persisted, current) => {
        const p = persisted as Partial<Pick<VeilState, "alarmEntries" | "activeTimers">> | null;
